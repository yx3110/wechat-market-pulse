...
走势图仅识别标的名称与代码。可接入本地 SQLite 行情库，由脚本计算日线指标，再由所选模型解释；生活照片、非行情截图仍完整解析。参见[本地行情配置](docs/LOCAL_MARKET.md)。

证券简称先结合群聊上下文扩展为正式名称，再核对代码并查行情；报告保留对应关系和推定依据，区分简称歧义、市场未适配与行情缺失。
...
手动报告按明确指定的群生成。配置里有多个群时必须传 `--group-id`，不会自动顺带生成其他群；只有显式重复传多个群才合并分析。定时任务使用各自已配置的群范围。

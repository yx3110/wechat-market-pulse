...
BRIEF_VERSION = "holistic-portable-v11"
...
不要逐条分析、不要发言排行榜、不要情绪百分比、不要把复读/闲聊当市场共识。按事件和个股合并同义线索，保留分歧。股票简称要结合同行业、产品、前后发言和回复理解；保留原文name/code供追溯，后续简称解析模块会核对正式全称与代码。不要把仅未写全称说成无法分析或本地无数据，也不要在每条分歧/建议里重复“先确认身份”。只有上下文确有不同主体冲突时才强调歧义。
...
limitations: 2至3项重要范围限制，不要泛泛免责声明。本部分是群聊资料归纳；如启用外部补查，行情和消息会独立列在标的旁边，不能预先声称已经核验。末尾的投资建议仅是基于群聊资料的条件性推断。截图里的代码和名称可原样记录，但不据此确认真实上市身份。用读者可理解的资料边界表述，不写“技术栏留空”“群元数据”等内部处理说明。

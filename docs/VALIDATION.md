...
- 单元测试：76 项，覆盖加密数据库、已提交 WAL、错误密钥、跨库导出、增量游标与未变化分库跳过、群昵称优先级、昵称缓存、图片解码、图片证据目录保留、多群完整性，以及日线计算、复权/标的/日期校验、消息来源、补查缓存和离线不联网。重点关注另覆盖同名账号/跨群隔离、观点变化的前后证据、无消息、改名缓存和只列出真实群成员。
